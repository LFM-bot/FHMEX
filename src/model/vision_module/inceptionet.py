...
class GoogLeNet(nn.Module):
    def __init__(self, aux_logits=False, num_classes=1000, use_SRM=False, antialias=True, device='cuda:0'):
        ...
        if self.use_SRM:
            ...
            self.bayar_mask = (torch.tensor(np.ones(shape=(5, 5)))).to(self.dev)
            self.bayar_mask[2, 2] = 0
            self.bayar_final = (torch.tensor(np.zeros((5, 5)))).to(self.dev)
...
class conv_block(nn.Module):
    def __init__(self, in_channels, out_channels, xbn=False, **kwargs):
        ...
        self.batchnorm = nn.BatchNorm2d(out_channels) if not xbn else nn.GroupNorm(num_channels=out_channels,
                                                                                   num_groups=16)
