...
class GlobalFilterBlock(nn.Module):
    ...
    def forward(self, sequence_emb, filter=None):
        sequence_emb = self.LayerNorm(sequence_emb)
        sequence_emb = self.dropout(sequence_emb)
        seq_mask = torch.ones(sequence_emb.shape[:-1], dtype=torch.bool).to(sequence_emb.device)
        # UAF
...
class LocalFilterBlock(nn.Module):
    def __init__(self, alpha=0.1, beta=0.2, n_layers=1, n_heads=2, hidden_size=64,
                 inner_size=256, hidden_dropout_prob=0., hidden_act='gelu', layer_norm_eps=1e-12,
                 initializer_range=0.02, kernel_size=7, MAX_ITEM_LIST_LENGTH=197, freq_dropout_prob=0.4,
                 num_bands=6, conv_layers=1):
        super(LocalFilterBlock, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.n_layers = n_layers
        self.n_heads = n_heads
...
class LFMfilterLayer(nn.Module):
    ...
    def forward(self, input_tensor, seq_mask, filter):
        ...
        frequency_bands = torch.empty((batch, self.num_bands, max_len, hidden), device=input_tensor.device,
