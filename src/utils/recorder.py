...
class Recorder:
    ...
    def _model_saving_init(self, config):
        ...
        if self.model_saved is None:
            self.model_saved = os.path.join(config.save, f'{config.model}-{self.dataset}-{self.mark}-{timestamp}.pth')
    ...
    def early_stop_check(self, core_metric_res, model):
        if core_metric_res > self.best_metric_rec.get('score'):
            ...
            logging.info(f'EarlyStopping Counter: {self.patience - self.step_2_stop} out of {self.patience}.' + \
                         f' Best at epoch {self.best_metric_rec["epoch"]}. Best {self.core_metric}: {round(self.best_metric_rec["score"], 4)}')
    ...
    def train_log_verbose(self, num_batch):
        ...
        total_men = round(meminfo.total / 1024 ** 3, 2)
        used_men = round(meminfo.used / 1024 ** 3, 2)
        free_men = round(meminfo.free / 1024 ** 3, 2)
        men_info = f'Memory Usage: Total: {total_men} GB\tUsed: {used_men} GB\tFree: {free_men} GB'
        logging.info(men_info)
    ...
    def report_test_result(self, test_metric_res):
        ...
        logging.info(res_str)
        if 'classification_report' in test_metric_res.keys():
            logging.info(test_metric_res['classification_report'])
